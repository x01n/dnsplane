...
import { Loader2, ArrowLeft, CheckCircle, ShieldOff } from 'lucide-react'
...
export default function ForgotTOTPPage() {
  ...
  const handleSendCode = async () => {
    ...
    setCodeSending(true)
    try {
      const res = await authApi.sendCode(email, 'forgot_totp')
      if (res.code === 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await authApi.forgotTotp({
        email,
        code,
    ...
    <div className="flex min-h-svh w-full items-center justify-center p-6 md:p-10">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center gap-2 mb-6">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-500 text-white"><ShieldOff className="h-5 w-5" /></div>
          <h1 className="text-xl font-bold">DNSPlane</h1>
        </div>
        <Card>
          <CardHeader className="text-center">
            <CardTitle className="text-xl">重置二步验证</CardTitle>
            <CardDescription>通过邮箱验证后关闭二步验证</CardDescription>
          </CardHeader>
          <CardContent>
            {step === 2 ? (
              <div className="text-center space-y-4">
                ...
                <p className="text-xs text-muted-foreground">点击链接后即可关闭二步验证。</p>
