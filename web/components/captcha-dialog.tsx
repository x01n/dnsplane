...
import CaptchaEngine from 'go-captcha-react'
...
/*
 * BehavioralCaptchaDialog 行为验证码弹窗组件
 * 功能：支持点选/滑动/旋转三种验证码，通过弹窗展示，验证通过后返回 verify_token
 */

interface BehavioralCaptchaDialogProps {
  open: boolean
  onClose: () => void
...
export default function BehavioralCaptchaDialog({ open, onClose, onSuccess }: BehavioralCaptchaDialogProps) {
  ...
  const loadCaptcha = useCallback(async () => {
    ...
    setError(false)
    try {
      const res = await authApi.getBehavioralCaptcha()
  ...
  const submitVerify = async (captchaType: string, answer: unknown) => {
    ...
    try {
      const res = await authApi.verifyBehavioralCaptcha({
