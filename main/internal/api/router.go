...
func SetupRouter(staticFS embed.FS) *gin.Engine {
	...
	api.Use(middleware.AuditLog())
	{
		...
		api.GET("/auth/captcha/behavioral", handler.GetBehavioralCaptcha)
		api.POST("/auth/captcha/behavioral/verify", handler.VerifyBehavioralCaptcha)
		...
		auth.Use(middleware.Auth())
		{
			...
			auth.POST("/domains/dns-check", handler.DnsCheck)
			auth.GET("/domains/categories", handler.GetDomainCategories)
			auth.POST("/domains/categories", handler.CreateDomainCategory)
			auth.POST("/domains/categories/:id", handler.UpdateDomainCategory)
			auth.POST("/domains/categories/:id/delete", handler.DeleteDomainCategory)
			auth.POST("/domains/category", handler.SetDomainCategory)
			...
			auth.GET("/domains/:id/record-groups", handler.GetRecordGroups)
			auth.POST("/domains/:id/record-groups/change", handler.ChangeRecordGroup)

			// 一键加速
			auth.POST("/domains/:id/accel", handler.AccelerateRecord)
			auth.POST("/domains/:id/accel/batch", handler.BatchAccelerateRecords)
			auth.GET("/domains/:id/accel/status", handler.GetAccelStatus)
			auth.GET("/system/accel/config", handler.GetAccelConfig)
			auth.POST("/system/accel/config", handler.UpdateAccelConfig)
			auth.GET("/system/accel/platforms", handler.GetAccelPlatforms)
			auth.GET("/system/accel/accounts", handler.ListAccelAccounts)
			auth.POST("/system/accel/apply-account", handler.ApplyAccountToAccel)
			auth.POST("/system/accel/eo/zones", handler.ListEOZones)
			auth.POST("/system/accel/esa/sites", handler.ListESASites)
			...
			auth.POST("/cloudflare/hostnames/batch-add/:id", handler.BatchAddCustomHostnames)
			auth.POST("/cloudflare/hostnames/batch-update/:id", handler.BatchUpdateCustomHostnames)
			auth.POST("/cloudflare/hostnames/batch-delete/:id", handler.BatchDeleteCustomHostnames)
			auth.POST("/cloudflare/hostnames/txt-targets/:id", handler.GetCustomHostnameTxtTargets)
			auth.POST("/cloudflare/hostnames/refresh/:id", handler.RefreshCustomHostname)
			auth.POST("/cloudflare/hostnames/setup-validation/:id", handler.SetupHostnameValidation)
			...
			auth.POST("/cloudflare/tunnels/public-hostnames/data/:id", handler.GetTunnelPublicHostnames)
			auth.POST("/cloudflare/tunnels/public-hostnames/save/:id", handler.SaveTunnelPublicHostname)
			auth.POST("/cloudflare/tunnels/public-hostnames/delete/:id", handler.DeleteTunnelPublicHostname)
